...
Donatur melakukan scan QR code atau klik link donasi.

Sistem menampilkan halaman Form Donate.

Donatur mengisi: Nama, Email, Pesan, Jumlah Donasi, Media share/GIF, dan Metode Bayar.

[PROSES KRITIKAL] Sistem memvalidasi input pesan (AI Moderation). Jika pesan ditolak, donatur kembali ke Form Donate dengan notifikasi error. Jika aman, lanjut ke pembayaran.

Proses Pembayaran via Payment Gateway.

Jika pembayaran gagal, donatur kembali ke proses pembayaran. Jika sukses, pesan dan animasi ditampilkan pada stream (Overlay).

Selesai.

3.2. Alur Streamer (Streamer Flowchart)
Login/Register akun Streamer.

Masuk ke halaman Dashboard (Menampilkan: Link/QR Donate, Saldo, dan Riwayat Donasi & Withdraw).

Navigasi Sidebar memberikan akses ke 4 menu utama:
